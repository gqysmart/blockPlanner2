...
gulp.task("html", ["inject", "partials"], function() {
    ...
    return gulp.src([path.join(conf.paths.tmp, conf.appName, "*.ejs")])
        ...
        .pipe(gulp.dest(path.join(conf.paths.dist, conf.appName, "assets/home")))
    ...
    .pipe(htmlFilter)
        ...
        // }))
        .pipe(gulp.dest(path.join(conf.paths.dist, conf.appName, "assets/home/views")))
...
gulp.task('fonts', function() {
    return gulp.src($plugins.mainBowerFiles('**/*.{eot,svg,ttf,woff,woff2}'))
        ...
        .pipe(gulp.dest(path.join(conf.paths.dist, conf.appName, 'public/home/fonts/')));
});
