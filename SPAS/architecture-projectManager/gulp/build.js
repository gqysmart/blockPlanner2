...
gulp.task("html", ["inject", "partials"], function() {
    ...
    return gulp.src([path.join(conf.paths.tmp, "*.ejs")])
        .pipe(useref({
        ...
        .pipe(gulp.dest(path.join(conf.paths.dist, "assets", conf.appName)))
    ...
    .pipe(htmlFilter)
        ...
        // }))
        .pipe(gulp.dest(path.join(conf.paths.dist, "assets", conf.appName, "views")))
...
gulp.task("revisionReplace", ["revision"], function() {
    ...
    // var manifest = gulp.src(path.join(conf.paths.tmp, "rev-mainfest.json"));
    // return gulp.src(path.join(conf.paths.tmp, "*.ejs"))
    //     .pipe($plugins.revReplace({
    //         manifest: manifest
    //     }))
...
gulp.task('fonts', function() {
    return gulp.src($plugins.mainBowerFiles('**/*.{eot,svg,ttf,woff,woff2}'))
        ...
        .pipe(gulp.dest(path.join(conf.paths.dist, "public", conf.appName, "fonts")));
});
